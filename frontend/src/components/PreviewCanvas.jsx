...
export default function PreviewCanvas({ state }) {
  ...
  // KEY FIX: keep latest state in a ref so the draw loop always reads current values
  const stateRef   = useRef(state);
  const assetsRef  = useRef({ particles: [], lasers: [], shockwaveRef: { current: null }, spinOffset: 0 });

  // Update stateRef every render without restarting the loop
  ...
  useEffect(() => {
    ...
    const draw = (ts) => {
      ...
      // Always read from ref — guaranteed fresh state
      const s = stateRef.current;
      const W = canvas.offsetWidth;
      const H = canvas.offsetHeight;

      const bI = 60 / Math.max(s.bpm || 128, 1);
      beatRef.current = (beatRef.current + dt / bI) % 1;

      if (s.mosaicRotate) spinRef.current += dt * (s.mosaicRotateSpeed || 1);
      assetsRef.current.spinOffset = spinRef.current;

      ctx.clearRect(0, 0, W, H);

      if (s.mode === 'mosaic') {
        drawPreviewGrid(ctx, W, H, timeRef.current, beatRef.current, s, spinRef.current);
      } else {
        drawFrame(ctx, W, H, timeRef.current, beatRef.current, dt, s, assetsRef.current, 0);
      }

      // Mini phone silhouettes overlay
      if (s.mode !== 'mosaic' && !s.blackout && s.mode !== 'blackout') {
        const rgb = hexToRgb(s.color || '#8b00ff');
        const bp  = Math.pow(Math.sin(beatRef.current * Math.PI), 3);
        const a   = lerp(0.5, 0.9, bp) * ((s.brightness || 88) / 100);
        for (let i = 0; i < 5; i++) {
          const dx = W / 6 * (i + 1);
          const dy = H / 2 + Math.sin(timeRef.current * 2.2 + i) * 4;
  ...
  }, []); // mount only — state read via stateRef
