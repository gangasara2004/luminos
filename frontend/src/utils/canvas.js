...
// ── SAFE WRAPPER — never lets a crash kill the animation loop ─────────────────
export function drawFrame(ctx, W, H, t, beatPhase, dt, state, assets, deviceId = 0) {
  try {
    _drawFrame(ctx, W, H, t, beatPhase, dt, state, assets, deviceId);
  } catch (e) {
...
function _drawFrame(ctx, W, H, t, beatPhase, dt, state, assets, deviceId) {
  const { particles, lasers, shockwaveRef } = assets;
  const br   = clamp((state.brightness || 88) / 100, 0, 1);
  const it   = clamp((state.intensity  || 75) / 100, 0, 1);
  const bp   = Math.pow(Math.sin(beatPhase * Math.PI), 3);
  const mode = state.blackout ? 'blackout' : (state.mode || 'breathing');
  ...
  const rgb  = hexToRgb(state.color  || '#8b00ff');
  const rgb2 = hexToRgb(state.color2 || '#00d4ff');
  ...
  if (mode === 'mosaic') {
    ...
    if (state.mosaicBeat) {
      for (let i = 0; i < 2; i++) {
        ...
        ctx.beginPath(); ctx.arc(W/2, H/2, ph*Math.max(W,H)*.8, 0, Math.PI*2);
        ctx.strokeStyle = `rgba(255,255,255,${(1-ph)*.55*br})`;
  ...
  // ── BACKGROUNDS
  if (mode === 'wave') {
    // Key fix: ensure color stops are strictly ascending 0..1
    const wp = ((t * .55) % 2 - .5); // -0.5 .. 1.5
    const s0 = 0;
    const s1 = clamp(wp,        0.01, 0.99);
    const s2 = clamp(wp + .35,  0.02, 0.99);
    const s3 = 1;
    // Only draw gradient if stops are valid ascending
    if (s1 < s2) {
      const wg = ctx.createLinearGradient(0, 0, W, 0);
      wg.addColorStop(s0, 'rgba(0,0,0,.95)');
      wg.addColorStop(s1, `rgba(${rgb.r},${rgb.g},${rgb.b},${br})`);
      wg.addColorStop(s2, `rgba(${rgb2.r},${rgb2.g},${rgb2.b},${.55*br})`);
      wg.addColorStop(s3, 'rgba(0,0,0,.95)');
      ctx.fillStyle = wg;
    } else {
      ctx.fillStyle = `rgba(${rgb.r},${rgb.g},${rgb.b},${br})`;
    }
    ctx.fillRect(0, 0, W, H);

  } else if (mode === 'strobe') {
    const on = (t * (state.strobeSpeed || 9)) % 1 > .5;
    ctx.fillStyle = on ? `rgba(${rgb.r},${rgb.g},${rgb.b},${br})` : 'rgba(0,0,0,.98)';
    ctx.fillRect(0, 0, W, H);
    ...
    else if (mode === 'breathing') a = lerp(.28, .82, (Math.sin(t * (state.bpm||128) / 140) + 1) / 2) * br;
  ...
  if (mode === 'galaxy' || mode === 'breathing' || mode === 'particles') {
    for (let i = 0; i < 120; i++) {
      ...
      ctx.beginPath(); ctx.arc(sx, sy, sz*(.5+fl*.5), 0, Math.PI*2);
      ctx.fillStyle = `rgba(255,255,255,${.25+fl*.75})`; ctx.fill();
    }
    if (mode === 'galaxy') {
  ...
  if (mode === 'laser' || mode === 'bassdrop') {
    ...
    lasers.forEach(l => {
      l.angle += l.speed;
      const cx = W/2, cy = H*.12, len = Math.sqrt(W*W+H*H);
      ctx.beginPath(); ctx.moveTo(cx,cy);
      ctx.lineTo(cx+Math.cos(l.angle)*len, cy+Math.sin(l.angle)*len);
      ctx.strokeStyle = `hsla(${l.hue},100%,65%,${.55*br*it})`;
      ctx.lineWidth = l.width; ctx.shadowColor=`hsl(${l.hue},100%,70%)`; ctx.shadowBlur=14; ctx.stroke();
      ctx.beginPath(); ctx.moveTo(cx,cy);
      ctx.lineTo(cx-Math.cos(l.angle)*len, cy+Math.sin(l.angle)*len);
      ctx.strokeStyle = `hsla(${(l.hue+45)%360},100%,58%,${.38*br*it})`; ctx.stroke();
    });
    ctx.restore();
  }
  ...
  if (mode==='particles'||mode==='galaxy'||mode==='rainbow'||mode==='breathing') {
    ...
    particles.forEach(p => {
      ...
      p.x += p.vx*(1+bp*1.8);
      p.y += p.vy*(1+bp*1.5);
      const lr=p.life/p.maxLife, a=Math.sin(lr*Math.PI)*br;
      const h=mode==='rainbow'?(p.hue+t*35)%360:p.hue;
      ctx.beginPath(); ctx.arc(p.x,p.y,p.size*(1-lr*.4),0,Math.PI*2);
      ctx.fillStyle=`hsla(${h},100%,72%,${a})`;
      ctx.shadowColor=`hsl(${h},100%,80%)`; ctx.shadowBlur=p.size*3; ctx.fill();
    });
    ctx.restore();
  }
  ...
  if (mode==='pulse'||mode==='bassdrop') {
    ...
    for (let i=0;i<rings;i++) {
      const ph=(beatPhase+i/rings)%1, r=ph*Math.max(W,H)*.9, a=(1-ph)*.65*br*it;
      ctx.beginPath(); ctx.arc(W/2,H/2,r,0,Math.PI*2);
      ctx.strokeStyle=`rgba(${rgb.r},${rgb.g},${rgb.b},${a})`;
      ctx.lineWidth=3*(1-ph); ctx.shadowColor=state.color; ctx.shadowBlur=18; ctx.stroke();
    }
    ctx.restore();
  }
  ...
  if (shockwaveRef && shockwaveRef.current) {
    ...
    if (el<0.65) {
      ...
      ctx.strokeStyle=`rgba(255,90,40,${a})`; ctx.lineWidth=9*(1-el/0.65);
      ctx.shadowColor='#ff4400'; ctx.shadowBlur=45; ctx.stroke(); ctx.restore();
    } else { shockwaveRef.current=null; }
  }
